...
class DeliveryAddress {
    ...
    @ApiPropertyOptional({ type: String, description: "Second address line (optional)" })
    @IsOptional()
    @IsString()
    address2?: string;
...
class Address {
    @ApiPropertyOptional({ type: String, description: "Copy from existing customer address ID" })
    @IsOptional()
    @IsString()
    copyFromCustomerAddressId?: string;

    @ApiProperty({ type: DeliveryAddress, description: "Delivery address details" })
    @ValidateNested()
...
export class CheckoutAddressDTO {
    @ApiProperty({ type: String, description: "The unique identifier of the cart" })
    @IsString()
    cartId: string;
